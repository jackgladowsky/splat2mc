...
def generate_mcfunction(
    ...
    Args:
        ...
        use_absolute_pos: If set, use absolute coordinates offset from this position
    """
    lines = [
...
def generate_datapack(
    ...
    Args:
        ...
        loop: Whether to create a looping tick function
...
tellraw @s {{"text":"Stopped splat display","color":"yellow"}}"""
    ...
    # Write tick function that checks if active
    tick_content = f"""# Check if splat should be displayed (runs every tick)
execute if score {safe_name} splat_active matches 1 run function splats:{safe_name}"""
